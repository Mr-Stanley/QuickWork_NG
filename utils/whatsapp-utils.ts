...
export function createWhatsAppAppUrl(phoneNumber: string, message?: string): string {
  const formattedNumber = formatPhoneForWhatsApp(phoneNumber)
  const encodedMessage = message ? encodeURIComponent(message) : ""

  return `whatsapp://send?phone=${formattedNumber}${message ? `&text=${encodedMessage}` : ""}`
}
...
export const messageTemplates = {
  ...
  quote: (providerName: string, category: string) =>
    `Hi ${providerName}! I found your profile on QuickWorkNG and would like to request a quote for ${category} services.`,
  ...
  support: () => `Hi! I need help with QuickWorkNG. Can you assist me?`,

  general: (providerName: string) =>
    `Hi ${providerName}! I found your profile on QuickWorkNG and would like to connect.`,
}
